...
class AjaxGetResourcePool extends AjaxAction {
  ...
  protected function _userRun() {
    if (($id = ifsetor($this->_params['id']))||($asset = ifsetor($this->_params['assetId']))) {
      ...
      $s->setColumnsMask(array('name','external_id','description','price','unit'));
      ...
      $s->setColumnsMask(array('resourcepool_id','name','resource_all'));
      #adump($s->toString());die;
      $res = $this->_app->db->doQuery($s->toString());
      $this->_result = array();
      while ($row = $this->_app->db->fetchAssoc($res)) {
        $row['id'] = $row['resourcepool_id'];
