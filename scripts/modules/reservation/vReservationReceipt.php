...
class ModuleReservationReceipt extends DocumentModule {
  ...
  protected function _userInsert() {
    ...
    $s->setColumnsMask(array('reservation_id','receipt'));
    $res = $this->_app->db->doQuery($s->toString());
    if (!$row = $this->_app->db->fetchAssoc($res)) die('Invalid reservation!');
    if (!$row['receipt']) die('Invalid reservation!');
    ...
    $s = new SCreditnote;
    $s->addStatement(new SqlStatementMono($s->columns['type'], "%s='RECEIPT'"));
