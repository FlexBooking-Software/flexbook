...
class ModuleUserCreditRefund extends ExecModule {

  protected function _userRun() {
    $user = $this->_app->request->getParams('id');
    if ($provider = $this->_app->request->getParams('provider')) {  
      if (!$change = $this->_app->request->getParams('creditRefund_'.$provider)) {
      ...
      $this->_app->messages->addMessage('userInfo', sprintf($this->_app->textStorage->getText('info.editUserCredit_returnOk'), $change,
