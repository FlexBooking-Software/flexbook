...
class GridSettingsDocumentTemplate extends WebGridSettings {
  ...
  protected function _initSettings() {
    ...
    $this->addColumn(new GridColumn('id', 'documenttemplate_id', 'ID', 'none'));
    $this->addColumn(new GridColumn('provider', 'provider_id', $ts->getText('label.listDocumentTemplate_provider'), 'select'));
    $this->addColumn(new GridColumn('providerName', 'provider_name', $ts->getText('label.listDocumentTemplate_provider'), 'none'));    
    $this->addColumn(new GridColumn('name', 'name', $ts->getText('label.listDocumentTemplate_name')));
    $this->addColumn(new GridColumn('target', 'target', $ts->getText('label.listDocumentTemplate_target')));
    $this->addColumn(new GridColumn('description', 'description', $ts->getText('label.listDocumentTemplate_description'), 'none'));
    $this->addColumn(new GridColumn('action', 'documenttemplate_id', $ts->getText('label.grid_none'), 'none'));
    
    if (in_array($this->_name, array('listDocumentTemplate'))) {
      ...
      $this->_onPage = $ONPAGE['listDocumentTemplate'];
    } elseif ($this->_name == 'selectDocumentTemplate') {
    ...
    $this->getColumn('i')->addGuiElement(new GuiGridCellID(array('columnName'=>'documenttemplate_id')));
...
class GuiGridCellTarget extends GuiGridCellRenderer {
  ...
  protected function _userRender() {
    $this->setTemplateString($this->_app->textStorage->getText('label.listDocumentTemplate_target_'.$this->_outputData));
  }
}

class GuiListDocumentTemplate extends GuiWebGrid {

  public function __construct($name='listDocumentTemplate') {
    $app = Application::get();
    $showFilter = true;
    $showPager = true;

    $gridSettings = new GridSettingsDocumentTemplate($name);
    $select = new SDocumentTemplate($gridSettings->getSqlSelectSettings());
